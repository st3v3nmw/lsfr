	...
	return New().
		...
		Test("PUT Basic Operations", func(do *Do) {
			...
			for country, capital := range capitals {
				do.HTTP("node", "PUT", fmt.Sprintf("/kv/%s:capital", country), capital).
		...
		Test("PUT Edge and Error Cases", func(do *Do) {
			...
			do.HTTP("node", "GET", "/kv/special:key-with_symbols.123").
		...
		Test("Concurrent Operations - Different Keys", func(do *Do) {
			...
			putFn := func(key, value string) func() {
				return func() {
					do.HTTP("node", "PUT", "/kv/concurrent:"+key, value).
						Returns().Status(Is(200)).
			...
			for i := 1; i <= 100; i++ {
				do.HTTP("node", "GET", fmt.Sprintf("/kv/concurrent:key%d", i)).
		...
		Test("Concurrent Operations - Same Key", func(do *Do) {
			...
			do.HTTP("node", "GET", "/kv/concurrent:racekey").
		...
		Test("Check Allowed HTTP Methods", func(do *Do) {
			...
			for _, method := range []string{"POST", "PATCH"} {
				do.HTTP("node", method, "/kv/test:key").
